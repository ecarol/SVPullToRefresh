...
- (void)layoutSubviews {
    ...
    float left = (self.bounds.size.width - self.titleLabel.frame.size.width -
                  self.activityIndicatorView.frame.size.width - 5.0f) / 2;
    CGRect activityFrame = self.activityIndicatorView.frame;
    activityFrame.origin.x = left;
    activityFrame.origin.y = self.bounds.size.height - (SVPullToRefreshViewMargin + activityFrame.size.height) / 2;
    self.activityIndicatorView.frame = activityFrame;
    
    self.titleLabel.center = self.activityIndicatorView.center;
    CGRect titleFrame = self.titleLabel.frame;
    titleFrame.origin.x = activityFrame.origin.x + activityFrame.size.width + 5.0f;
